  #Enter the martix Ex:V<-makeVector(c(4,3,3,2),r,c);r=rows;c=columns; If the matrix does not have an inverse it will throw an error
...
makeCacheMatrix <- function(x = numeric(),r,c) # input x will be a vector
  ...
  get <- function() k                       # returns the value of the original matrix
...
# This function will check if the inverse is already stored,if its stored then returns inverse else it calculates inverse and returns inverse 
